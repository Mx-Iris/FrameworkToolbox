...
**Re-export chain — important when moving code between these targets.** `OSToolbox` re-exports `FrameworkToolbox` and `os`; `SwiftStdlibToolbox` re-exports `OSToolbox` **and `DyldToolbox`**; `FoundationToolbox` re-exports `SwiftStdlibToolbox` and `Combine`. That is why `import FoundationToolbox` alone resolves `@Mutex`, `@Loggable`, `AccessLevel`, `os.Logger`, and `@DyldInterpose`. `@_exported import` carries macro declarations *and* their plugins across transitive dependencies, so a macro can be moved into another target without breaking any existing call site — see `Documentations/Specs/2026-08-06-ostoolbox-extraction-design.md` and `Documentations/Evolutions/0001-dyld-toolbox-extraction.md`.

A re-export is invisible in the sense that nothing fails when it breaks — no test covers "the old import path still works". Both extractions therefore pin it at compile time from a client target: `Sources/OSToolboxClient/LoggableWithoutFoundation.swift` for the `OSToolbox` split, and the `DyldToolbox re-export` section at the end of `Sources/SwiftStdlibToolboxClient/main.swift` for this one. Keep that pattern when moving anything else down a layer.

## Architecture

### Macro System

Each library has a corresponding macro target (`*Macros`). A shared `MacroToolbox` target provides reusable protocols for lock-style macros (`LockMacroProtocol`, `LockPropertyParser`) — `DyldToolboxMacros` deliberately does not depend on it, since none of that machinery applies to interposing.
