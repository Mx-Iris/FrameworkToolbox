...
public enum LoggableMacro {
    ...
    /// Key-path root type for the `category:` argument of `#log`.
    ///
    /// `#log(.debug, category: \.network, "…")` type-checks its key-path
    /// argument against this type: the String-keyed dynamic member subscript
    /// accepts any member name, and the macro reads that name from the key-path
    /// syntax at compile time, re-resolving it as a case of the `LogCategory`
    /// enum generated by `@Loggable(categories:)` on the enclosing type. The
    /// key path itself is discarded during expansion and never evaluated at
    /// runtime.
    @dynamicMemberLookup
    public struct Categories {
        public subscript(dynamicMember categoryName: String) -> Category { Category() }
    }

    /// Value type produced by ``Categories``' dynamic member subscript.
    ///
    /// Never instantiated at runtime — it exists only so category key paths
    /// type-check before macro expansion replaces them.
...
/// Overload of `#log` that logs through one of the categories declared via
/// `@Loggable(categories:)` on the enclosing type.
///
/// The `category:` argument must be a key-path literal naming a declared
/// category. The key path is purely syntactic transport: the macro extracts
/// the member name at compile time and resolves it against the generated
/// `LogCategory` enum, so a name that was never declared fails to compile.
///
/// Example:
///
...
///     // }()
///
/// > Note: Because the member lookup is dynamic, the IDE does not autocomplete
/// > category names after `\.` — the compile-time check happens when the
/// > expansion resolves the name against the `LogCategory` enum.
@freestanding(expression)
public macro log(
